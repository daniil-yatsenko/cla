// some code from GitHub from 2023-24. Presumably a cleaner version of what is stored in raw-code-backup.html
// most probably won't work as-is.
...
function homeProjectList(page) {
  ...
  projectLinks = page.querySelectorAll(".image-wrapper.is-project-list");
  ...
  if (window.innerWidth > 991) {
    ...
    projects.forEach((project) => {
      ...
      if (!clonedTitle.classList.contains("is-coming-soon")) {
        ...
        gsap.set(clonedTitle.children[0], { display: "inline-block" });
...
function alignGrids(page = document) {
  if (window.innerWidth > 1300) {
    ...
    document.querySelectorAll(".project_info-wrapper-v2").forEach((grid) => {
      let children = grid.children;
      for (let i = 0; i < children.length; i++) {
        if ((i + 1) % 2 === 0) {
          let origX = children[i].getBoundingClientRect().left;
          let delta = serviceLinkX - origX;
          if (delta > 10) {
            children[i].style.transform = `translateX(${delta}px)`;
            children[i].style.width = `${children[i].offsetWidth - delta}px`;
          }
        }
      }
    });
...
function logoAlignLeftMobile(animate = true) {
  if (window.innerWidth < 992) {
    ...
    tl.set(logo.parentElement, { gridColumn: "1 / 3", justifySelf: "start" });
...
barba.init({
  views: [
    {
      ...
      afterEnter(data) {
        ...
        homeInit(container);
        gsap.to(document.querySelector(".page-wrapper"), { opacity: 1 });
  ...
  transitions: [
    ...
    },
    {
      ...
      leave(data) {
        openOverlayFunction();
        gsap.to(data.current.container, { opacity: 0, duration: 0.3 });
        setTimeout(() => {
          window.scrollTo(0, 0);
